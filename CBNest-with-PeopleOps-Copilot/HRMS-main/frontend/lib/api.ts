...
// ---------------------------------------------------------------------------
// PeopleOps Copilot (AI chat) — Policy RAG / SQL Agent / HR Action Agent
// ---------------------------------------------------------------------------
...
export async function askPolicy(
  token: string,
  message: string
): Promise<ApiResult<ApiEnvelope<PolicyChatData> | { detail?: unknown }>> {
...
export async function askSql(
  token: string,
  message: string
): Promise<ApiResult<ApiEnvelope<SqlChatData> | { detail?: unknown }>> {
...
): Promise<ApiResult<ApiEnvelope<ActionChatData> | { detail?: unknown }>> {
  const response = await fetch(`${API_BASE}/api/v1/chat/actions`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    body: JSON.stringify({ message, pending_action: pendingAction ?? null }),
  });
  return { ok: response.ok, status: response.status, body: await response.json() };
}

// ---------------------------------------------------------------------------
// PeopleOps Copilot — AI Observability (admin-only)
// ---------------------------------------------------------------------------
...
export type AiObservabilityStatusRow = { status: string; count: number };
export type AiObservabilityToolRow = { tool_name: string; count: number };
export type AiObservabilityRoleRow = { role: string; count: number };
export type AiObservabilityRefusalReasonRow = { reason: string; count: number };

export type AiObservabilityDailyRow = {
  date: string;
  count: number;
  avg_latency_ms: number | null;
  input_tokens: number;
...
export type AiObservabilityData = {
  since: string;
  days: number;
  totals: AiObservabilityTotals;
  by_intent: AiObservabilityIntentRow[];
  by_status: AiObservabilityStatusRow[];
  by_tool: AiObservabilityToolRow[];
  by_role: AiObservabilityRoleRow[];
  by_refusal_reason: AiObservabilityRefusalReasonRow[];
  daily: AiObservabilityDailyRow[];
};

export async function fetchAiObservability(
  token: string,
  days: number = 14
): Promise<ApiResult<ApiEnvelope<AiObservabilityData> | { detail?: unknown }>> {
  const response = await fetch(`${API_BASE}/api/v1/chat/observability?days=${days}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  return { ok: response.ok, status: response.status, body: await response.json() };
}
